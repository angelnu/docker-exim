...
echo -n "" > /etc/exim4/conf.d/main/00_local_macros

if [ "$MAILNAME" ]; then
	echo "MAIN_HARDCODE_PRIMARY_HOSTNAME = $MAILNAME" > /etc/exim4/conf.d/main/00_local_macros
	echo $MAILNAME > /etc/mailname
fi

if [ "$KEY_PATH" -a "$CERTIFICATE_PATH" ]; then
	if [ "$MAILNAME" ]; then
	  echo "MAIN_TLS_ENABLE = yes" >>  /etc/exim4/conf.d/main/00_local_macros
	else
	  echo "MAIN_TLS_ENABLE = yes" >>  /etc/exim4/conf.d/main/00_local_macros
	fi
...
if [ "$DISABLE_IPV6" ]; then
        echo 'disable_ipv6=true' >> /etc/exim4/conf.d/main/00_local_macros
fi
...
if [ -f /etc/exim4/_docker_additional_macros ]; then
	cat /etc/exim4/_docker_additional_macros >> /etc/exim4/conf.d/main/00_local_macros
fi
